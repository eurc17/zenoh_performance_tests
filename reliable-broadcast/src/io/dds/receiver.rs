...
{
    pub(crate) reader: Option<DataReader<T, CDRDeserializerAdapter<T>>>,
}
...
{
    pub async fn recv_sample(&mut self) -> Result<Option<DataSample<T>>> {
    ...
    pub async fn recv(&mut self) -> Result<Option<T>> {
        ...
        let data = sample.map(|s| s.into_value());
        Ok(data)
    }

    pub fn into_sample_stream(self) -> impl Stream<Item = Result<DataSample<T>>> {
