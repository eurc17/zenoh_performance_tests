...
def main(args):
    ...
    for json_file in sorted(glob.glob(args.input_dir + "/*.json"), key=sort_file_key):
        ...
        print(file)
        init_time = int(os.path.basename(file).split("-")[-1])
        ...
        peer_list.append(init_time)
        mem_list.append(mem_usage)
        cpu_list.append(cpu_usage)
        print(init_time, mem_usage, cpu_usage)
    fig, ax1 = plt.subplots()

    color = "tab:red"
    ax1.set_xlabel("init time (ms)", fontweight="bold")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run multiple zenoh-performance-test with increasing initialization time. RUST_LOG=info must be set"
    )
    parser.add_argument(
        "-o",
        "--output_dir",
